...
struct SplitTests {
    ...
    @Test func `split parts have correct measurements`() async throws {
        ...
        let topVolume = await topMeasurements.volume
        #expect(topVolume ≈ 800.0)
        ...
        let bottomVolume = await bottomMeasurements.volume
        #expect(bottomVolume ≈ 200.0)
    ...
    @Test func `3D split by single axis range produces correct halves`() async throws {
        ...
        let upperVolume = await upper.volume
        #expect(upperVolume ≈ 500)
        ...
        let lowerVolume = await lower.volume
        #expect(lowerVolume ≈ 500)
    ...
    @Test func `3D split by closed range extracts a slab`() async throws {
        ...
        let slabVolume = await slab.volume
        #expect(slabVolume ≈ 600)
        ...
        let restVolume = await rest.volume
        #expect(restVolume ≈ 400)
    }
    ...
    @Test func `3D split with multi-axis ranges extracts a sub-box`() async throws {
        ...
        let insideVolume = await inside.volume
        #expect(insideVolume ≈ 360)
    ...
    @Test func `2D split by single axis range produces correct halves`() async throws {
        ...
        let upperArea = await upper.area
        #expect(upperArea ≈ 50)
        ...
        let lowerArea = await lower.area
        #expect(lowerArea ≈ 50)
    ...
    @Test func `2D split with multi-axis ranges extracts a sub-rectangle`() async throws {
        ...
        let insideArea = await inside.area
        #expect(insideArea ≈ 36)
    ...
    @Test func `2D split with mask separates inside and outside`() async throws {
        ...
        let insideArea = await inside.area
        #expect(insideArea.equals(circleArea, within: 0.05))
        ...
        let outsideMeasuredArea = await outside.area
        #expect(outsideMeasuredArea.equals(outsideArea, within: 0.05))
    }
