...
struct BezierPathTests {
    ...
    @Test func linearAllPoints() {
        let points = linearPath.points(segments: .fixed(10))
        let points2 = linearPath.points(segments: .defaults)
    ...
    @Test func linearAllPointsClosed() {
        let points = linearPath.closed().points(segments: .fixed(10))
        #expect(points ≈ (linearPoints + [linearPoints[0]]))
    }

    @Test func linearLimitedRange() {
        let rangePoints = linearPath.points(in: 0.5...1.5, segments: .fixed(10))
        let rangePoints2 = linearPath.points(in: 0.5...1.5, segments: .defaults)
    ...
    @Test func linearExtendedRange() {
        let extendedRangePoints = linearPath.points(in: -0.5...2.5, segments: .fixed(10))
    ...
    @Test func quadraticPointsFixed() {
        let points = quadraticPath.points(segments: .fixed(5))
    ...
    @Test func quadraticPointsDynamic() {
        let points = quadraticPath.points(segments: .dynamic(minAngle: 10°, minSize: 20))
