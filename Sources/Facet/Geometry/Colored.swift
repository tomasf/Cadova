...
public extension Geometry3D {
    func withMaterial(color: Color, metallicness: Double, roughness: Double, name: String? = nil) -> any Geometry3D {
        ApplyMaterial(body: self, material: .init(name: name, baseColor: color, properties: .metallic(metallicness: metallicness, roughness: roughness)))
    }

    func withMaterial(color: Color, specular: Color, glossiness: Double, name: String? = nil) -> any Geometry3D {
        ApplyMaterial(body: self, material: .init(name: name, baseColor: color, properties: .specular(color: specular, glossiness: glossiness)))
    }

    func withMaterial(_ material: Material) -> any Geometry3D {
        ApplyMaterial(body: self, material: material)
    }
}
