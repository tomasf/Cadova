...
`Model` and `Project` are meant as top-level calls for model executables: they run your geometry and write the result to disk. For scripts, GUI applications, servers, or other contexts where you need control over where and how files are saved, use ``ModelFileGenerator`` instead.
