...
internal struct MeshData: Sendable, Hashable, Codable {
    ...
    internal init(vertices: [Vector3D], faces: [Face], faceMaterials: FaceMaterials? = nil) {
        ...
        // A mesh without materials digests as it always has, so existing goldens keep their order.
        if let faceMaterials = self.faceMaterials {
            hasher.combine(faceMaterials)
        }
    ...
    private enum CodingKeys: String, CodingKey {
        case vertices, faces, faceMaterials
    }
    ...
    internal init(from decoder: Decoder) throws {
        ...
        self.init(
            ...
            faceMaterials: try container.decodeIfPresent(FaceMaterials.self, forKey: .faceMaterials)
        )
    }
    ...
    internal func encode(to encoder: Encoder) throws {
        ...
        try container.encodeIfPresent(faceMaterials, forKey: .faceMaterials)
    }
...
internal extension MeshData {
    ...
    struct FaceMaterials: Sendable, Hashable, Codable, StableHashable {
