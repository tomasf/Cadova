...
public extension Geometry2D {
    ...
    ///
    /// This method assumes that the geometry is rectangular or approximately rectangular, and applies an edge profile to
    /// the specified corners. The profile determines the shape of the corners — for example, a chamfer or a fillet.
    ///
    /// - Parameters:
...
public extension Geometry3D {
    ...
    ///
    /// - Returns: A new `Geometry3D` object with the edge profile applied to the specified corners along the given axis.
    ///
    /// This method uses the bounding box of the geometry to determine the appropriate size and position for the profile.
    /// It is intended for geometries that are box-like or similar enough for this approximation to be effective.
    /// The shape of fillets is determined by the environment’s `roundedCornerStyle`, which controls whether corners
    /// are shaped as simple circular arcs or smoother, squircle-like transitions.
    ///
    func applyingEdgeProfile(_ edgeProfile: EdgeProfile, to corners: Rectangle.Corners = .all, along axis: Axis3D) -> any Geometry3D {
