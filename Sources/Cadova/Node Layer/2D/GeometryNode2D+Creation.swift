...
public extension GeometryNode2D {
    static var empty: GeometryNode2D {
        Self(.empty)
    }

    static func boolean(_ children: [GeometryNode2D], type: BooleanOperationType) -> GeometryNode2D {
    ...
    static func convexHull(_ body: GeometryNode2D) -> GeometryNode2D {
        guard body.isEmpty == false else { return .empty }
        return Self(.convexHull(body))
    }

    static func shape(_ shape: PrimitiveShape) -> GeometryNode2D {
        guard shape.isEmpty == false else { return .empty }
        return Self(.shape(shape))
    }

    static func transform(_ body: GeometryNode2D, transform: AffineTransform2D) -> GeometryNode2D {
        guard body.isEmpty == false else { return .empty }
    ...
    static func materialized(cacheKey: OpaqueKey) -> GeometryNode2D {
        return Self(.materialized(cacheKey: cacheKey))
    }

    static func offset(_ body: GeometryNode2D, amount: Double, joinStyle: LineJoinStyle, miterLimit: Double, segmentCount: Int) -> GeometryNode2D {
    ...
    static func projection(_ body: GeometryNode3D, type: Projection) -> GeometryNode2D {
        guard !body.isEmpty else { return .empty }
