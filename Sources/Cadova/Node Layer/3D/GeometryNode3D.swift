...
public struct GeometryNode3D: GeometryNode, Sendable {
    ...
    indirect enum Contents {
        ...
        case boolean ([GeometryNode3D], type: BooleanOperationType)
        case transform (GeometryNode3D, transform: AffineTransform3D)
        case convexHull (GeometryNode3D)
        case materialized (cacheKey: OpaqueKey)
        case applyMaterial (GeometryNode3D, Material)
        case extrusion (GeometryNode2D, type: Extrusion)
        case lazyUnion ([GeometryNode3D])
    }
...
extension GeometryNode3D {
    public var isEmpty: Bool {
        if case .empty = contents { true } else { false }
    }

    public func evaluate(in context: EvaluationContext) async -> EvaluationResult<D> {
        switch contents {
        case .empty:
            return .empty
        ...
        case .extrusion (let expression, let extrusion):
            ...
            case .linear (let height, let twist, let divisions, let scaleTop):
                Result(result.concrete.extrude(height: height, divisions: divisions, twist: twist.degrees, scaleTop: scaleTop))

            case .rotational (let angle, let segmentCount):
                Result(result.concrete.revolve(degrees: angle.degrees, circularSegments: segmentCount))
            }

        case .lazyUnion (let members):
            let results = await context.results(for: members)
            return Result(product: .init(composing: results.map(\.concrete)), results: results)
        }
    }
}
